...
- **`_parameter_modmode` restricted via `setattr`**: Values 1 (Unipolar), 2 (Bipolar), 3 (Additive) silently reset to 0. Only 0 (None) and 4 (Absolute) persist. Verified in both standalone Max AND Ableton Live (2026-05-25) — not a context issue but a `setattr` API limitation. These modes can only be set via the Inspector GUI.
